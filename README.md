...
```bash
# 1. Stage the bundled Audiveris + JRE + Tesseract for the current OS
#    (downloads ~200MB the first time)
scripts/fetch_runtime_linux.sh        # Linux x86_64
# scripts/fetch_runtime_macos.sh      # macOS arm64 / x64
# pwsh -File scripts/fetch_runtime_windows.ps1   # Windows x64
...
# 5. Or build installers for the current platform
npm run tauri:build
```
...
```bash
# 1. 現在の OS 用に Audiveris + JRE + Tesseract を同梱位置に配置
#    （初回は ~200MB ダウンロード）
scripts/fetch_runtime_linux.sh                  # Linux x86_64
# scripts/fetch_runtime_macos.sh                # macOS arm64 / x64
# pwsh -File scripts/fetch_runtime_windows.ps1  # Windows x64

# 2. Python サイドカーを frontend/src-tauri/bin/ にビルド
pip install pyinstaller
pip install -e backend
scripts/build_sidecar.sh

# 3. （初回のみ）1024x1024 の PNG からアイコン生成
cd frontend
npx @tauri-apps/cli icon path/to/source.png -o src-tauri/icons
